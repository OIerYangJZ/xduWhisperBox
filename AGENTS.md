...
- 新增全局黑名单（屏蔽用户）功能，完成前后端联调、SQLite 数据库模式迁移与单元测试
- 微信小程序端 Markdown 渲染器大幅升级，全面解析表格、代码块、链接、粗斜体、列表等，并配套优化了渲染 CSS 样式
- 修复小程序多端运行时对 `isDevtoolsRuntime` 的判定 Bug 并在 AI 模型切换页面修复 Loading/Toast 冲突问题
- 微信小程序端个人中心、公开主页、编辑资料、账号安全、隐私、通知、会话等页面已继续向移动端主 App 对齐
- 微信小程序端私信申请流程已移除，改为与主 App 一致的直接会话逻辑
- 微信小程序端默认头像图兜底已移除，匿名帖在卡片与详情页展示“匿名”标签且头像显示“匿”
...
- 小程序匿名帖必须一眼可识别：头像显示“匿”，作者信息旁展示小型“匿名”标签，且头像不跳转个人主页
- 小程序私信逻辑与主 App 对齐：不再使用私信申请；隐私设置允许私信且帖子非匿名时可直接进入会话
...
1. 将当前通知中心、全局黑名单、Markdown 渲染、小程序对齐等改动部署到腾讯云现网
2. 继续增强管理员后台（批量审核、更强的过滤/搜索、全量数据导出）
3. 中期仍需补生产化能力
   - 域名 + HTTPS
   - 备案
   - 备份 / 回滚

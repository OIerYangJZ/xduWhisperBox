...
write_miniprogram_local_config() {
  local file="$ROOT_DIR/miniprogram/config/env.local.js"
  [[ -n "$LAN_ORIGIN" ]] || return 0
  [[ -d "$ROOT_DIR/miniprogram/config" ]] || return 0

  cat > "$file" <<EOF
// Auto-generated by scripts/start_backend.sh. Do not commit.
const localConfig = {
  env: 'auto',
  devtoolsBaseUrl: 'http://localhost:$BACKEND_PORT',
  lanBaseUrl: '$LAN_ORIGIN',
};
...
if [[ -n "$LAN_ORIGIN" ]]; then
  ...
  echo "[backend]   Mini config      : miniprogram/config/env.local.js"
